...
//    [self addSubview:_topView];
    ...
    //弹幕
    _msgTableView = [[TCMsgListTableView alloc] initWithFrame:CGRectMake(15, 0, MSG_TABLEVIEW_WIDTH, self.height - 40) style:UITableViewStyleGrouped];
    ...
    //输入框
    _msgInputView = [[UIView alloc] initWithFrame:CGRectMake(0, SCREEN_HEIGHT - 45 - TabbarSafeBottomMargin, self.width, MSG_TEXT_SEND_VIEW_HEIGHT )];
    ...
    bulletBtn.frame = CGRectMake(10, (_msgInputView.height - MSG_TEXT_SEND_FEILD_HEIGHT)/2, MSG_TEXT_SEND_FEILD_HEIGHT, MSG_TEXT_SEND_FEILD_HEIGHT);
    [bulletBtn setImage:[UIImage imageNamed:@"选择图片"] forState:UIControlStateNormal];
    [bulletBtn setImage:[UIImage imageNamed:@"选择图片"] forState:UIControlStateSelected];
//    [bulletBtn addTarget:self action:@selector(clickBullet:) forControlEvents:UIControlEventTouchUpInside];
    [bulletBtn addTarget:self action:@selector(photoAction:) forControlEvents:UIControlEventTouchUpInside];
    
    self.bulletBtn1 = [UIButton buttonWithType:UIButtonTypeCustom];
        self.bulletBtn1.frame = CGRectMake(10 + 30, (_msgInputView.height - MSG_TEXT_SEND_FEILD_HEIGHT)/2, MSG_TEXT_SEND_FEILD_HEIGHT, MSG_TEXT_SEND_FEILD_HEIGHT);
        [self.bulletBtn1 setImage:[UIImage imageNamed:@"图片"] forState:UIControlStateNormal];
    self.bulletBtn1.selected = NO;
        [self.bulletBtn1 addTarget:self action:@selector(selectArrAction:) forControlEvents:UIControlEventTouchUpInside];
//        [bulletBtn addTarget:self action:@selector(clickBullet:) forControlEvents:UIControlEventTouchUpInside];
...
//    [sendBtn setTitle:@"发送" forState:UIControlStateNormal];
    [sendBtn setImage:[UIImage imageNamed:@"fasong"] forState:normal];
    ...
    msgInputFeildLine1.frame = CGRectMake(self.bulletBtn1.right + 10, sendBtn.y, 1, MSG_TEXT_SEND_FEILD_HEIGHT);
...
- (void)selectArrAction:(UIButton *)sender{
    // 筛选数组
    sender.selected = !sender.isSelected;
    [_msgTableView changeActionAll:sender.isSelected];
...
//        }
    ...
    [[V2TIMManager sharedInstance] sendMessage:msg receiver:nil groupID:[[ProfileManager shared]roomID]
...
- (void)keyboardFrameDidChange:(NSNotification *)notice {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        if (shouldHidden) {
            self->_msgInputView.frame = CGRectMake(0, SCREEN_HEIGHT - 45 - TabbarSafeBottomMargin, self.width, MSG_TEXT_SEND_VIEW_HEIGHT );
            if (self.msgTableView.isBig == YES) {
                self->_msgInputView.hidden = YES;
                self->_msgInputView.frame = CGRectMake(0, self.height - 45, self.width, MSG_TEXT_SEND_VIEW_HEIGHT );
            }else{
                self->_msgInputView.hidden = NO;
            }
...
//        [HUDHelper alert:@"消息不能为空"];
    ...
    NSDictionary *tempDic = @{
     @"chatRoomNumber":[NSString stringWithFormat:@"%@",[[ProfileManager shared]roomID]],//
     @"chatSenderId":[NSString stringWithFormat:@"%@",[[[ProfileManager shared] loginUserModel]userId]],
     @"chatSenderName":[NSString stringWithFormat:@"%@",[[ProfileManager shared] roomNickName]],
     @"chatRecipientId":@"0",
     @"chatMessage":_msgInputFeild.text
    };
    [HPNetManager POSTWithUrlString:@"http://39.106.88.75:9999/TeacherLive/pages/room/sendChatMessage" isNeedCache:NO parameters:tempDic successBlock:^(id response) {
    ...
    if (_bulletBtnIsOn) {
        ...
        [_liveRoom sendRoomCustomMsgWithCommand:[@(TCMsgModelType_DanmaMsg) stringValue] message:textMsg callback:^(int code, NSString * error) {
            if ([error containsString:@"secure check error"]) {
                [HPProgressHUD showMessage:@"含有敏感词发送失败"];
            }else if([error containsString:@"you have been forbidden to speak"]){
                [HPProgressHUD showMessage:@"您已被禁言,无法发送"];
            }else{
                [self bulletMsg:msgModel];
            }
        }];
