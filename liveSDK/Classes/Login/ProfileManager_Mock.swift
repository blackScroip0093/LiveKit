...
@objc public class ProfileManager: NSObject {
    ...
    @objc public func autoLogin(success: @escaping ()->Void,
        ...
        if let cacheData = UserDefaults.standard.object(forKey: tokenKey) as? Data {
            do {
                ...
                login(success: success, failed: fail, auto: false)
                return false
            } catch {
    ...
    @objc public func login(success: @escaping ()->Void,
        ...
        let phoneValue = ProfileManager.shared.loginUserModel?.userId;
        if !auto {
            assert(phoneValue?.count ?? 0 > 0)
            let random = arc4random_uniform(999999999) + 1
//            curUserModel = LoginResultModel(userID: "\(random)")
