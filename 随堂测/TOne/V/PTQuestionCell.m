...
//    NSDictionary *opArr = model.option_xuan;
    
     for (NSDictionary *opArr in model.option_xuan) {
            if (opArr.count >1) {
    //            [choiceArr addObject:[opArr objectAtIndex:1]];
                [choiceArr addObject:opArr[@"answerContent"]];
            }
        }
    
//    if (opArr.count > 0) {
//        [choiceArr addObject:opArr[@"answerSelect"]];
//    }
    ...
    NSInteger minutes = [startStr integerValue];
    [self.choiceView setChoiceData:self.choiceDesc index:minutes];
...
//        [self.scrollV addSubview:self.nextBtn];
        [self.lastBtn setHidden:YES];
        [self setConstraints];
    }else {
//        [self.scrollV addSubview:self.lastBtn];
//        [self.scrollV addSubview:self.nextBtn];
        [self.lastBtn setHidden:NO];
        [self setConstraints];
    }
...
- (void)setConstraints {
    ...
    [self.typeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(32, 0));
    }];
...
//    }
    ...
    self.scrollV.contentSize = CGSizeMake(self.width, 123+HP_SCALE_H(30)+HP_SCALE_H(60) * _choiceDesc.count+self.titleLabel.height - 80);
...
- (UILabel *)titleLabel {
    ...
    if (!_titleLabel) {
        _titleLabel = [UILabel initLabelTextFont:18 textColor:[UIColor hex:@"262626"] title:@""];
...
- (UIButton *)lastBtn {
    if (!_lastBtn) {
        _lastBtn = [UIButton initButtonTitleFont:14 titleColor:[UIColor hex:@"1890FF"] titleName:@"上一题" backgroundColor:[UIColor whiteColor] radius:3.0];
...
- (UIButton *)nextBtn {
    ...
    if (!_nextBtn) {
//        _nextBtn = [UIButton initButtonTitleFont:14 titleColor:[UIColor hex:@"1890FF"] titleName:@"下一题" backgroundColor:[UIColor whiteColor] radius:3.0];
        _nextBtn = [UIButton buttonWithType:UIButtonTypeSystem];
        _nextBtn.tag = 100011;
        [_nextBtn setTitle:@"下一题" forState:normal];
        [_nextBtn setTitleColor:[UIColor hex:@"1890FF"] forState:normal];
